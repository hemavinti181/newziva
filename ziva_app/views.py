...
def wh_item_list(request):
    ...
    if response.status_code == 200:
        ...
        return render(request,'stock_transfer/stock_transfer_home.html',{'type':type,'taxinvoice':taxinvoice,'warehouseinventorylist':warehouseinventorylist,'warehouselist':warehouselist[0],'wh_item_list':wh_item_list,'menuname':menuname,'data':stocktransferlistto,'wh':'active'})
    elif response.status_code == 400:
        data = response.json()
        messages.error(request, data['message'])
        return render(request, 'login1.html')
        ...
        return render(request, 'stock_transfer/stock_transfer_home.html',
                      {'type':type,'warehouseinventorylist' : warehouseinventorylist, 'menuname': menuname,'data':stocktransferlistto,'taxinvoice':taxinvoice,'wh':'active','wh':'active','warehouselist':warehouselist[0]})
...
def depot_dashboard(request):
    try:
        ...
        if response.status_code == 200:
            ...
            wh_masterlist = data['warehouselist']
            if role == 'Admin':
                    return render(request, 'dashboard/depot_admin_dashboard.html', {"menuname": menuname,'wh_masterlist':wh_masterlist,'item_masterlist':item_masterlist})
            else:
...
def bust_dashboard(request):
    try:
        ...
        if response.status_code == 200:
            ...
            wh_masterlist = data['warehouselist']
            if role == 'Admin':
                return render(request, 'dashboard/bust_admin_dashboard.html', {"menuname": menuname, 'wh_masterlist': wh_masterlist,'item_masterlist':item_masterlist})
            else:
                return render(request, 'dashboard/bust_dashboard.html',
                              {"menuname": menuname, 'wh_masterlist': wh_masterlist, 'item_masterlist': item_masterlist})
        elif response.status_code == 400:
            data = response.json()
            messages.error(request, data['message'])
            return redirect('login')
...
def zonal_dashboard(request):
    try:
            ...
            if response.status_code == 200:
                data = response.json()
                depolist = data['depolist']
                return render(request, 'dashboard/zonal_dashboard.html', {"menuname": menuname,'depolist':depolist,'item_masterlist':item_masterlist})
            elif response.status_code == 400:
                data = response.json()
                messages.error(request,data['message'])
                return redirect('/login')
...
def zonal_depotindent_data(request):
    accesskey = request.session['accesskey']
    url = "http://13.235.112.1/ziva/mobile-api/depo-pending-indent.php"
    fdate = request.POST.get('cdate')
    tdate = datetime.date.today()
    tdate = tdate.strftime("%Y-%m-%d")
    if fdate:
